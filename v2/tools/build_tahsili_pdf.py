...
def draw_cover(c):
    ...
    draw_en(c, '188', cx, 200, font='PlexB', size=56, color=BRIGHT_GOLD, align='center')
    draw_ar(c, 'سؤال محلول · بشرح بسيط', cx, 182, font='PlexSB', size=10.5, color=WHITE, align='center')

    # chips
    chips_y = 140
    subjects = [('الرياضيات', '41'), ('الفيزياء', '46'), ('الكيمياء', '47'), ('الأحياء', '54')]
    cw = 72
    total_w = cw * 4 + 5 * 3
    sx = (PAGE_W - total_w) / 2
...
def draw_header_footer(c, page_num, total, subject_label=''):
    ...
    c.setFont('PlexM', 8)
    c.drawString(15, 8, f'{page_num} / {total}')
    draw_ar(c, 'madarekelite.com', PAGE_W - 15, 8, font='PlexSB', size=8.5, color=PRI, align='right')
    if subject_label:
        draw_ar(c, subject_label, PAGE_W/2, 8, font='Plex', size=8, color=INK_3, align='center')
    c.restoreState()
...
def estimate_question_height(c, q_data):
    ...
    # 1. شريط الرقم + المادة (بادج)
    h = 26 + 8
    # 2. نص السؤال — احسب أسطر مع size 11
    qtext = q_data['question_text']
    qfont, qsize = 'PlexSB', 11
    max_w = card_w - 24
    qlines = wrap_text(c, qtext, qfont, qsize, max_w)
    h += len(qlines) * (qsize + 5) + 14 + 8
    # 3. خيارات 2x2
    h += (32 + 5) * 2 + 2
    # 4. صندوق الشرح
    expl = q_data.get('explanation', '')
    if expl:
        ex_lines = wrap_text(c, expl, 'Plex', 8.5, card_w - 78)
        h += len(ex_lines) * 11 + 20 + 8
    # هامش بين الأسئلة
    h += 6
    return h
...
def draw_question(c, q_num, q_data, y_top):
    """رسم سؤال بدءاً من y_top. يرجع y النهائي."""
    margin = 16
    card_x = margin
    card_w = PAGE_W - 2 * margin
    cur_y = y_top

    # شريط رقم السؤال + المادة·الموضوع
    bar_h = 26
    c.setFillColor(HERO_1)
    c.roundRect(card_x, cur_y - bar_h, card_w, bar_h, 7, fill=1, stroke=0)
    # بادج الرقم (ذهبي pill) — أرقام لاتينية
    c.setFillColor(GOLD)
    c.roundRect(card_x + 8, cur_y - bar_h + 5, 32, bar_h - 10, 8, fill=1, stroke=0)
    draw_en(c, str(q_num), card_x + 8 + 16, cur_y - bar_h + 11, font='PlexB', size=11, color=HERO_1, align='center')

    # المادة · الموضوع (يمين)
    subj_topic = q_data['subject'] + (' · ' + q_data['topic'] if q_data.get('topic') else '')
    draw_ar(c, subj_topic, card_x + card_w - 10, cur_y - bar_h + 11, font='PlexSB', size=9, color=WHITE, align='right')
    cur_y -= bar_h + 8
    ...
    qbox_h = len(lines) * (qsize + 5) + 14
    c.setFillColor(WHITE)
    c.setStrokeColor(LINE)
    c.setLineWidth(0.7)
    c.roundRect(card_x, cur_y - qbox_h, card_w, qbox_h, 6, fill=1, stroke=1)
    for i, ln in enumerate(lines):
        draw_ar(c, ln, card_x + card_w - 12, cur_y - 15 - i*(qsize+5), font=qfont, size=qsize, color=INK, align='right')
    cur_y -= qbox_h + 8

    # الخيارات 2x2 grid (RTL: أ يمين، ب يسار)
    ...
    for i in range(4):
        col = 1 - (i % 2)  # i=0 (أ) → right
        row = i // 2
        cx = card_x + col * (col_w + 8)
        cy = cur_y - row * (row_h + 5) - row_h
        ...
        c.setFillColor(bg)
        c.setStrokeColor(br)
        c.setLineWidth(1.4 if is_correct else 0.7)
        c.roundRect(cx, cy, col_w, row_h, 6, fill=1, stroke=1)

        # دائرة الحرف يمين الكارد
        letter_bg = SUC if is_correct else SOFT
        letter_fg = WHITE if is_correct else INK_3
        c.setFillColor(letter_bg)
        c.circle(cx + col_w - 13, cy + row_h/2, 8, fill=1, stroke=0)
        draw_ar(c, letters[i], cx + col_w - 13, cy + row_h/2 - 3, font='PlexB', size=9, color=letter_fg, align='center')

        # نص الخيار
        ch_text = str(choices[i])
        # تدريج الحجم لو طويل
        max_ch_w = col_w - 30
        for sz in [9.5, 9, 8.5, 8]:
            if c.stringWidth(shape(ch_text), 'PlexM', sz) <= max_ch_w:
                draw_ar(c, ch_text, cx + col_w - 26, cy + row_h/2 - 3, font='PlexM', size=sz, color=ink, align='right')
                break
        else:
            draw_ar(c, ch_text, cx + col_w - 26, cy + row_h/2 - 3, font='PlexM', size=7.5, color=ink, align='right')
    ...
    if expl:
        ex_lines = wrap_text(c, expl, 'Plex', 8.5, card_w - 78)
        ex_box_h = len(ex_lines) * 11 + 20

        c.setFillColor(YELLOW_BG)
        c.setStrokeColor(YELLOW_BR)
        c.setLineWidth(0.6)
        c.roundRect(card_x, cur_y - ex_box_h, card_w, ex_box_h, 5, fill=1, stroke=1)
        # label
        draw_ar(c, '◆ الشرح', card_x + card_w - 10, cur_y - 13, font='PlexB', size=9, color=YELLOW_INK, align='right')
        # text
        for i, ln in enumerate(ex_lines):
            draw_ar(c, ln, card_x + card_w - 10, cur_y - 27 - i*11, font='Plex', size=8.5, color=HexColor('#451A03'), align='right')

        # QR على اليسار
        qr_size = 40
        qr_x = card_x + 8
        qr_y = cur_y - ex_box_h + 8
        qr_img = make_qr(f"https://madarekelite.com/q/{q_data.get('id','')}")
        c.drawImage(ImageReader(qr_img), qr_x, qr_y, qr_size, qr_size, mask='auto')
...
if __name__ == '__main__':
    ...
    if '--sample' in sys.argv:
        ...
        build('/tmp/madarek_pdf/tahsili_full.pdf', sample_only=False)
