...
# ─── الخطوط ───
FONT_DIR = '/tmp/madarek_pdf/fonts'  # reuse Wednesday's fonts
...
def draw_cover(c):
    ...
    draw_ar(c, 'اليوم الثاني · الخميس ١٤ مايو', cx, badge_y + 10, font='PlexB', size=12, color=PRI_DEEP, align='center')
...
def draw_header_footer(c, page_num, total, subject_label=''):
    ...
    # عنوان (يسار)
    draw_ar(c, 'تسريبات التحصيلي · اليوم الثاني', 12, PAGE_H - 20, font='PlexM', size=8.5, color=HexColor('#E5E1FF'), align='left')
    c.restoreState()
...
def build(out_path, sample_only=False):
    ...
    with open('/tmp/thu_leak/all_q_clean.json') as f:
        all_q = json.load(f)
...
if __name__ == '__main__':
    ...
    if '--sample' in sys.argv:
        ...
        build('/tmp/thu_leak/tahsili_thu.pdf', sample_only=False)
