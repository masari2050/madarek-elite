...
def draw_cover(c):
    ...
    draw_ar(c, 'سؤال محلول · امسح QR للشرح', cx, 182, font='PlexSB', size=10.5, color=WHITE, align='center')
...
def estimate_question_height(c, q_data):
    ...
    qfont, qsize = 'PlexSB', 10.5
    max_w = card_w - 20
    qlines = wrap_text(c, qtext, qfont, qsize, max_w)
    h += len(qlines) * (qsize + 4) + 10 + 5  # نص السؤال
    h += (26 + 4) * 2 + 2  # خيارات 2x2
    h += 30 + 5  # QR bar
    h += 5  # هامش
    return h
...
def draw_question(c, q_num, q_data, y_top):
    """v3 compact: 3 questions/page, no explanation, QR for site."""
    margin = 14
    card_x = margin
    card_w = PAGE_W - 2 * margin
    cur_y = y_top

    # شريط رقم + مادة·موضوع — أصغر
    bar_h = 22
    c.setFillColor(HERO_1)
    c.roundRect(card_x, cur_y - bar_h, card_w, bar_h, 6, fill=1, stroke=0)
    c.setFillColor(GOLD)
    c.roundRect(card_x + 6, cur_y - bar_h + 4, 28, bar_h - 8, 7, fill=1, stroke=0)
    draw_en(c, str(q_num), card_x + 6 + 14, cur_y - bar_h + 9, font='PlexB', size=10, color=HERO_1, align='center')
    subj_topic = q_data['subject'] + (' · ' + q_data['topic'] if q_data.get('topic') else '')
    draw_ar(c, subj_topic, card_x + card_w - 8, cur_y - bar_h + 9, font='PlexSB', size=8.5, color=WHITE, align='right')
    cur_y -= bar_h + 5
    ...
    qbox_h = len(lines) * (qsize + 4) + 10
    c.setFillColor(WHITE)
    c.setStrokeColor(LINE)
    c.setLineWidth(0.6)
    c.roundRect(card_x, cur_y - qbox_h, card_w, qbox_h, 5, fill=1, stroke=1)
    for i, ln in enumerate(lines):
        draw_ar(c, ln, card_x + card_w - 10, cur_y - 13 - i*(qsize+4), font=qfont, size=qsize, color=INK, align='right')
    cur_y -= qbox_h + 5
    ...
    for i in range(4):
        col = 1 - (i % 2)
        row = i // 2
        cx = card_x + col * (col_w + 6)
        cy = cur_y - row * (row_h + 4) - row_h
        ...
        c.setFillColor(bg)
        c.setStrokeColor(br)
        c.setLineWidth(1.2 if is_correct else 0.6)
        c.roundRect(cx, cy, col_w, row_h, 5, fill=1, stroke=1)

        letter_bg = SUC if is_correct else SOFT
        letter_fg = WHITE if is_correct else INK_3
        c.setFillColor(letter_bg)
        c.circle(cx + col_w - 11, cy + row_h/2, 7, fill=1, stroke=0)
        draw_ar(c, letters[i], cx + col_w - 11, cy + row_h/2 - 3, font='PlexB', size=8.5, color=letter_fg, align='center')

        ch_text = str(choices[i])
        max_ch_w = col_w - 26
        for sz in [9, 8.5, 8, 7.5]:
            if c.stringWidth(shape(ch_text), 'PlexM', sz) <= max_ch_w:
                draw_ar(c, ch_text, cx + col_w - 22, cy + row_h/2 - 3, font='PlexM', size=sz, color=ink, align='right')
                break
        else:
            draw_ar(c, ch_text, cx + col_w - 22, cy + row_h/2 - 3, font='PlexM', size=7, color=ink, align='right')

    cur_y -= (row_h + 4) * 2 + 2

    # شريط QR للشرح — مدمج وأصغر
    cta_h = 30
    c.setFillColor(HexColor('#FEFCE8'))
    c.setStrokeColor(HexColor('#FDE68A'))
    c.setLineWidth(0.5)
    c.roundRect(card_x, cur_y - cta_h, card_w, cta_h, 5, fill=1, stroke=1)
    qr_size = 24
    qr_img = make_qr(f"https://madarekelite.com/q/{q_data.get('id','')}")
    c.drawImage(ImageReader(qr_img), card_x + 4, cur_y - cta_h + 3, qr_size, qr_size, mask='auto')
    draw_ar(c, '◆ امسح للشرح المفصّل على المنصّة', card_x + card_w - 10, cur_y - 13, font='PlexB', size=9, color=HexColor('#92400E'), align='right')
    draw_ar(c, 'madarekelite.com/q/' + str(q_data.get('id',''))[:8], card_x + card_w - 10, cur_y - 24, font='Plex', size=7, color=HexColor('#A16207'), align='right')
...
if __name__ == '__main__':
    ...
    if '--sample' in sys.argv:
        ...
        build('/tmp/madarek_pdf/tahsili_v3.pdf', sample_only=False)
